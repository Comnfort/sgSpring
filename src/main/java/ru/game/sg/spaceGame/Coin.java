        ...
        if (aL.size()!=0)
        {
            for (int i = 0; i < aL.size(); i++)
            {
                if (gf.getShip().getSh().contains(aL.get(i).getCenterX(), aL.get(i).getCenterY()))
                {
                    ...
                    new Sound(gf); countCoin+=1;  aL.remove(i);
                    if (countCoin==8){
   ...
   private void createCoin()
   {
       if (gf.getShip().getCountDeaths()==0){
           ...
           for (int i = 1; i < 4; i++)
           {
               ...
               for (int j = 1; j < 4; j++)
               {
                   aL.add(0, new Ellipse2D.Double(80 * j+(380*(i-1)), 40+(20 * i)+r, 28, 28));
               }
           }
           ...
           for (int i = 1; i < 4; i++)
           {
               for (int j = 1; j < 4; j++)
               {
                   aL.add(0, new Ellipse2D.Double((gf.getFieldWidth()*3-250)+(50 * i), 450+(50 * j), 28, 28));
               }
           }
